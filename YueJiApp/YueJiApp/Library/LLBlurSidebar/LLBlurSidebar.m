...
- (void)viewDidLoad {
    ...
    // 列表
    CGRect rect = CGRectMake(-self.leftMenuWidth, 0, self.leftMenuWidth, self.view.bounds.size.height);
...
#pragma mark - 显示/隐藏
- (BOOL)isSidebarShown{
    return self.contentView.frame.origin.x > -self.leftMenuWidth ? YES :NO;
}

- (void)showHideSidebar
{

    if (self.contentView.frame.origin.x == -self.leftMenuWidth) {
        LLLog(@"开始时在-点");
...
- (void)autoShowHideSidebar
{
    ...
    else
    {
        LLLog(@"自动缩回");
        [UIView animateWithDuration:duration animations:^{
            [self setSidebarOriginX:-self.leftMenuWidth];
        } completion:^(BOOL finished) {
            _isMoving = NO;
            self.view.hidden = YES;
        }];
...
/*
 * 设置侧栏位置
 * 完全不显示时为x=-self.homeLeftMenuWidth，显示到最右时x=0
 */
...
- (void)setSidebarOffset:(CGFloat)offset
{
    ...
    if (offset >=0) { // 右滑
        ...
        // 如果不在最左
        if (rect.origin.x > -self.leftMenuWidth) {
            rect.origin.x = startContentOriginX + offset;
            if (rect.origin.x < -self.leftMenuWidth) {
                rect.origin.x = -self.leftMenuWidth;
            }
        }
    }
    [self.contentView setFrame:rect];
...
{
    CGRect rect = self.contentView.frame;
    float percent = (self.leftMenuWidth + rect.origin.x) / self.leftMenuWidth;
    ...
    self.contentView.backgroundColor = [UIColor colorWithRed:red green:green blue:blue alpha:0.9];
}
